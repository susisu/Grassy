...
-- identity function
identity :: IxTerm
identity = IxAbs (IxVar 0)


-- A-normalization (+ elimination of applications with abstractions)
data EvalCtx =
    EmptyCtx
  | LetValCtx IxTerm
...
-- simplification + elimination of single variables except 0
elimVars' :: IxTerm -> IxTerm
elimVars' x@(IxVar 0)           = x
elimVars' x@(IxVar _)           = IxLet identity $ IxApp (IxVar 0) (shift 0 1 x)
...
liftLambdas' (IxAbs x) =
    case liftLambdas' x of
      -- lift if the body begins with an abstraction
      IxLet s@(IxAbs _) t
        | s `contains` 0 ->
          -- need closure conversion
          IxLet (IxAbs s) $ liftLambdas'
            $ IxAbs
              $ IxLet (IxApp (IxVar 1) (IxVar 0)) (shift 2 1 t)
        | otherwise ->
          IxLet (shift 0 (-1) s) $ liftLambdas'
            $ IxAbs (swap 0 1 t)
      x' -> IxAbs x'
liftLambdas' x@(IxApp _ _) = x
liftLambdas' (IxLet x y) =
    case liftLambdas' x of
      IxLet s t ->
        -- linearize
        liftLambdas' $ IxLet s $ IxLet t (shift 1 1 y)
      x'@(IxAbs _) ->
        case liftLambdas' y of
          y'@(IxAbs _) ->
            liftLambdas'
              $ IxLet x'
                $ IxLet y'
                  $ IxLet identity $ IxApp (IxVar 0) (IxVar 1)
          y' -> IxLet x' y'
      x' ->
        case liftLambdas' y of
          IxLet u@(IxAbs _) v
            ...
            | u `contains` 0 ->
              -- need closure conversion
              IxLet (IxAbs u) $ liftLambdas'
                $ IxLet (shift 0 1 x')
                  $ IxLet (IxApp (IxVar 1) (IxVar 0)) (shift 2 1 v)
            | otherwise ->
              IxLet (shift 0 (-1) u) $ liftLambdas'
                $ IxLet (shift 0 1 x') (swap 0 1 v)
          y'@(IxAbs _) ->
            liftLambdas'
              $ IxLet x'
                $ IxLet y'
...
toIndexed ctx (Var pos name) =
    case findIndex ctx name of
...
plantTerm' cs (IxApp (IxVar i) (IxVar j)) =
    return $ replicate (i + 1) (upperW cs) ++ replicate (j + 1) (lowerW cs)
...
plantTerm cs (IxLet x@(IxApp _ _) y@(IxLet (IxApp _ _) _)) =
    (++) <$> plantTerm' cs x <*> plantTerm cs y
